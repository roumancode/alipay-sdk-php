...
class AlipayComplainService extends AlipayService
{
     ...
     * @throws Exception
     */
    public function query(string $complain_event_id): mixed
    {
     ...
     * @throws Exception
     */
    public function batchQuery(?string $status = null, ?string $begin_time = null, ?string $end_time = null, int $page_num = 1, int $page_size = 10): mixed
    {
        ...
        if ($begin_time !== null) {
            $bizContent['begin_time'] = $begin_time;
        }
        if ($end_time !== null) {
            $bizContent['end_time'] = $end_time;
        }
        return $this->aopExecute($apiName, $bizContent);
    }
    ...
    {
        $image_type = pathinfo($file_name, PATHINFO_EXTENSION);
        if (empty($image_type)) {
            $image_type = 'png';
        }
    ...
    public function feedbackSubmit(string $complain_event_id, string $feedback_code, string $feedback_content, ?string $feedback_images = null): bool
    {
        ...
        if ($feedback_images !== null) {
            $bizContent['feedback_images'] = $feedback_images;
        }
    ...
    public function replySubmit(string $complain_event_id, string $reply_content, ?string $reply_images = null): bool
    {
        ...
        if ($reply_images !== null) {
            $bizContent['reply_images'] = $reply_images;
        }
        $this->aopExecute($apiName, $bizContent);
        return true;
    }
    ...
    public function supplementSubmit(string $complain_event_id, string $supplement_content, ?string $supplement_images = null): bool
    {
        ...
        if ($supplement_images !== null) {
            $bizContent['supplement_images'] = $supplement_images;
        }
        $this->aopExecute($apiName, $bizContent);
        return true;
    }
     ...
     * @throws Exception
     */
    public function riskquery(string $complain_id): mixed
    {
     ...
     * @throws Exception
     */
    public function riskbatchQuery(?string $status = null, ?string $begin_time = null, ?string $end_time = null, int $page_num = 1, int $page_size = 10): mixed
    {
     ...
     * @throws Exception
     */
    public function riskimageUpload(string $file_path, string $file_name): mixed
    {
    ...
    public function riskfeedbackSubmit(string $complain_id, string $process_code, string $remark, ?array $img_file_list = null): bool
    {
        ...
        if ($img_file_list !== null) {
            $bizContent['img_file_list'] = $img_file_list;
        }
        $this->aopExecute($apiName, $bizContent);
        return true;
    }
