...
{
    //AopClient
    protected AopClient $client;

    //是否公钥证书模式
    protected bool $isCertMode = false;
    ...
    //页面跳转接口返回类型
    protected ?string $pageMethod = null;
     ...
     * @throws Exception
     */
    public function aopExecute(string $apiName, ?array $bizContent = null, ?array $params = null): mixed
    {
    ...
    public function aopPageExecute(string $apiName, ?array $bizContent = null, ?array $params = null): string
    {
        ...
        if (!empty($this->pageMethod)) {
            $httpmethod = match ($this->pageMethod) {
                '2' => 'REDIRECT',
                '1' => 'GET',
                default => 'POST',
            };
     ...
     * 记录日志
     */
    public function writeLog(mixed $text): void
    {
        if (empty($this->logPath)) {
            return;
        }
        file_put_contents($this->logPath . "log.txt", date("Y-m-d H:i:s") . "  " . (string)$text . "\r\n", FILE_APPEND);
    }
}