...
class AlipayTransferService extends AlipayService
{
     ...
     * @throws Exception
     */
    public function transferToAccount(string $out_biz_no, float|string $amount, int $is_userid, string $payee_account, string $payee_real_name, string $payer_show_name): mixed
    {
        if ($this->isCertMode) {
            $apiName = 'alipay.fund.trans.uni.transfer';
            $payee_type = match ($is_userid) {
                2 => 'ALIPAY_OPEN_ID',
                1 => 'ALIPAY_USER_ID',
                default => 'ALIPAY_LOGON_ID',
            };
            $bizContent = [
                ...
                'payee_info' => ['identity' => $payee_account, 'identity_type' => $payee_type],
                'business_params' => json_encode(['payer_show_name_use_alias' => 'true'], JSON_UNESCAPED_UNICODE),
            ];
            if (!empty($payee_real_name)) {
                $bizContent['payee_info']['name'] = $payee_real_name; //收款方真实姓名
            }
        } else {
            ...
            if (!empty($payee_real_name)) {
                $bizContent['payee_real_name'] = $payee_real_name; //收款方真实姓名
            }
        }

        $result = $this->aopExecute($apiName, $bizContent);
        if (isset($result['pay_date'])) {
     ...
     * @throws Exception
     */
    public function transferToBankCard(string $out_biz_no, float|string $amount, string $payee_account, string $payee_real_name, string $payer_show_name): mixed
    {
        $apiName = 'alipay.fund.trans.uni.transfer';
        $bizContent = [
            'out_biz_no' => $out_biz_no, //商户转账唯一订单号
     ...
     * @throws Exception
     */
    public function query(string $order_id, int $type = 0, int $code = 0): mixed
    {
    ...
    /**
     * 账户余额查询
     * @param string|null $alipay_user_id 支付宝用户ID
     * @param int $user_type 用户标识类型（0支付宝UID,1支付宝openid）
     * @return mixed {"available_amount":"账户可用余额","freeze_amount":"实时冻结余额"}
     * @throws Exception
     */
    public function accountQuery(?string $alipay_user_id = null, int $user_type = 0): mixed
    {
     ...
     * @param string|null $original_order_id 原支付宝业务单号
     * @return mixed {"out_biz_no":"商户订单号","order_id":"支付宝转账订单号","pay_fund_order_id":"支付宝支付资金流水号","status":"SUCCESS","trans_date":"订单支付时间"}
     * @throws Exception
     */
    public function redPacketTansfer(string $out_biz_no, float|string $amount, string $user_id, string $order_title, ?string $original_order_id = null): mixed
    {
        $apiName = 'alipay.fund.trans.uni.transfer';
        $bizContent = [
            ...
            'payee_info' => ['identity' => $user_id, 'identity_type' => 'ALIPAY_USER_ID'],
            'business_params' => json_encode(['sub_biz_scene' => 'REDPACKET'], JSON_UNESCAPED_UNICODE)
        ];
        if ($original_order_id !== null) {
            $bizContent['original_order_id'] = $original_order_id;
        }
     ...
     * @param float|string $refund_amount 需要退款的金额
     ...
     * @throws Exception
     */
    public function redPacketRefund(string $out_request_no, string $order_id, float|string $refund_amount): mixed
    {
